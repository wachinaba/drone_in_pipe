...
class OpticalFlowVectorEstimator {
    public:
        ...
        void update_optical_flow(const gazebo_ros_optical_flow_msgs::OpticalFlow::ConstPtr& optical_flow_msg, const std::shared_ptr<const Eigen::Isometry3d>& transform, float range);
        ...
        Eigen::Matrix3d get_optical_flow_vector_covariance_matrix() {return optical_flow_vector_covariance_matrix_;};

    private:
        gazebo_ros_optical_flow_msgs::OpticalFlow optical_flow_msg_;
...
void OpticalFlowVectorEstimator::update_optical_flow(const gazebo_ros_optical_flow_msgs::OpticalFlow::ConstPtr& optical_flow_msg, const std::shared_ptr<const Eigen::Isometry3d>& transform, float range) {
    ...
    optical_flow_vector_covariance_matrix_ = (transform->rotation()) * covariance_matrix * (transform->rotation().transpose());

}

Eigen::Vector3d OpticalFlowVectorEstimator::calc_optical_flow_vector(const std::shared_ptr<const Eigen::Isometry3d>& transform, float range) {
    // calculate optical flow vector
    Eigen::Vector3d optical_flow_vector = Eigen::Vector3d(-optical_flow_msg_.integrated_x, -optical_flow_msg_.integrated_y, 0).array().tan() * range;
...
class SingleOpticalFlowSpeedEstimatorNode {
    public:
        SingleOpticalFlowSpeedEstimatorNode(std::string optical_flow_topic, std::string imu_topic, std::string range_topic, std::string base_link, std::string optical_flow_frame, std::string imu_frame, Eigen::Matrix2d optical_flow_covariance_matrix);
    ...
    private: 
        ...
        void OpticalFlowCallback(const gazebo_ros_optical_flow_msgs::OpticalFlow::ConstPtr& msg);
...
SingleOpticalFlowSpeedEstimatorNode::SingleOpticalFlowSpeedEstimatorNode(std::string optical_flow_topic, std::string imu_topic, std::string range_topic, std::string base_link, std::string optical_flow_frame, std::string imu_frame, Eigen::Matrix2d optical_flow_covariance_matrix) : 
            nh_("~"), 
            optical_flow_msg_(gazebo_ros_optical_flow_msgs::OpticalFlow()), 
...
void SingleOpticalFlowSpeedEstimatorNode::OpticalFlowCallback(const gazebo_ros_optical_flow_msgs::OpticalFlow::ConstPtr& msg) {
    ...
    Eigen::Matrix3d optical_flow_vector_covariance_matrix = optical_flow_vector_estimator_->get_optical_flow_vector_covariance_matrix();
    ...
    std::copy(covariance_matrix.data(), covariance_matrix.data() + covariance_matrix.size(), linear_velocity_msg.twist.covariance.begin());
...
int main(int argc, char **argv)
{
    ...
    SingleOpticalFlowSpeedEstimatorNode single_optical_flow_speed_estimator_node("/adrien/optical_flow_bottom_link/optical_flow", "/mavros/imu/data", "/single_point_lidar_bottom_link/range", "base_link", "optical_flow_bottom_link", "imu_link", Eigen::Matrix2d::Identity());
    ros::spin();
}
